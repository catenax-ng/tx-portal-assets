# Development / Deployment (CI/CD)

## Git diagram

```mermaid
%%{init: { 'logLevel': 'debug', 'theme': 'base' } }%%
gitGraph
    commit id: "release: v1.0.0" tag:"1.0.0"
    branch dev
    checkout dev
    branch feature/feature1
    commit id:"feat(function): add feature1"
    checkout dev
    branch bug/bug1
    commit id: "fix(function): change bug1"
    checkout dev
    branch feature/feature2
    commit id:"feat(function)!: enable feature2"
    checkout bug/bug1
    commit id:"fix(function): refactor bug1"
    checkout dev
    merge bug/bug1
    checkout dev
    branch feature/feature3
    commit id:"feat(function): wip - enable feature3"
    checkout dev
    checkout feature/feature3
    commit id: "feat(function): enable feature3"
    checkout feature/feature1
    commit id:"feat(function): enable feature1"
    checkout dev
    merge feature/feature1
    checkout dev
    merge feature/feature3
    checkout dev
    branch release/1.1.0
    commit id: "release(1.1.0-RC1): prepare migration (backend)"
    commit id: "release(1.1.0-RC1): bump version, update changelog..."
    checkout main
    merge release/1.1.0 tag: "1.1.0-RC1"
    checkout dev
    merge main
    checkout dev
    branch feature/feature4
    commit id: "feat(function): add feature4"
    checkout feature/feature2
    commit id: "feat(function)!: change feature2"
    checkout dev
    merge feature/feature2
    checkout feature/feature4
    commit id: "feat(function): change feature4"
    checkout dev
    merge feature/feature4
    checkout release/1.1.0
    merge dev
    commit id: "release(1.1.0-RC2): prepare migration (backend)"
    commit id: "release(1.1.0-RC2): bump version, update changelog..."
    checkout main
    merge release/1.1.0 tag: "1.1.0-RC2"
    checkout dev
    merge main
    checkout release/1.1.0
    branch bug/bug1-1.1.0
    commit id:"fix(function): change bug1-1.1.0"
    checkout release/1.1.0
    branch bug/bug2-1.1.0
    commit id:"fix(function): change bug2-1.1.0"
    checkout release/1.1.0
    merge bug/bug2-1.1.0
    checkout bug/bug1-1.1.0
    commit id:"fix(function): change2 bug1-1.1.0"
    checkout release/1.1.0
    merge bug/bug1-1.1.0
    commit id: "release(1.1.0-RC3): prepare migration (backend)"
    commit id: "release(1.1.0-RC3): bump version, update changelog..."
    checkout main
    merge release/1.1.0 tag: "1.1.0-RC3"
    checkout dev
    merge main
```

## Development flow with Deployment (CI/CD)

<p align="center">
<img width="1392" alt="image" src="https://user-images.githubusercontent.com/94133633/220213106-aef633bf-5995-4931-9533-fcfe225e7ec8.png">
</p>
